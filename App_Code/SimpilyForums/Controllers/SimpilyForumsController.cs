...
namespace Jumoo.Simpily
{
    ...
    public class SimpilyForumsController : SurfaceController
    {
        ...
        public ActionResult PostReply([Bind(Prefix="Reply")]SimpilyForumsPostModel model)
        {
            ...
            {
                ModelState.AddModelError("Resposta", "Erro ao postar (invalid model)");
                return CurrentUmbracoPage();
            }

            if (!CanPost(model))
            {
                ModelState.AddModelError("Resposta", "Você não tem permissões para postar aqui");
                return CurrentUmbracoPage();
            }
            ...
            {
                ModelState.AddModelError("Resposta", "Erro de postagem (bloqueado)");
                return CurrentUmbracoPage();
            }
            ...
            if (parent != null)
            {
                ...
                if (post != null )
                {
                    post.SetValue("DiscussionForumPosting_name", model.Title);
                    post.SetValue("DiscussionForumPosting_articleBody", model.Body);

                    var author = Members.GetById(model.AuthorId);
                    if (author != null)
                    {
                        post.SetValue("DiscussionForumPosting_creator", author.Name);
                        post.SetValue("DiscussionForumPosting_author", author.Id);
                    }
                    ...
                    if (parent.ContentType.Alias != "SimpilyForum")
                    {
                        ...
                        post.SetValue("DiscussionForumPosting_replies_", true);
                    }
            ...
            ModelState.AddModelError("Resposta", "Erro ao criar a postagem");
            return RedirectToCurrentUmbracoPage();
        }
        ...
        private bool CanPost(SimpilyForumsPostModel model)
        {
            ...
            if ( model.ParentId > 0 ) 
            {
                ...
                if ( parent != null )
                {
                    var canPostGroups = parent.GetPropertyValue<string>("Forum_groupsAllowed_", true);
