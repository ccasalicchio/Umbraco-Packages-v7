    ...
    public class ForumHelper
    {
        ...
        public static string GetRelativeDate(DateTime date)
        {
            ...
            if (span.Days > 0)
            {
                ...
                {
                    return string.Format("{0} dia atrás", span.Days);
                }
            }
            else if (span.Hours > 0)
            {
                return string.Format("{0} hora{1} atrás", span.Hours, span.Hours > 1 ? "s" : "");
            }
            else if (span.Minutes > 0)
            {
                return string.Format("{0} minuto{1} atrás", span.Minutes, span.Minutes > 1 ? "s" : "");
            }
            else
            {
